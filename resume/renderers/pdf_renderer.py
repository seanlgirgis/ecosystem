try:
    ...
    print("WARNING: pdfkit not installed. PDF generation disabled.")
...
class PdfRenderer:
    def __init__(self, theme):
        ...
        try:
            ...
            print(f"WARNING: wkhtmltopdf not found at {self.wkhtmltopdf_path}")
            self.config = None
        
    def render_from_html(self, html_content, output_path, footer_config=None):
        if not PDFKIT_AVAILABLE or not self.config:
            print("PDF generation skipped (pdfkit or wkhtmltopdf not available)")
            return
        options = {
            'page-size': 'Letter',
            'margin-top': '0mm', 
